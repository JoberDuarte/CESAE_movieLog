...
final class TMDBService {
    ...
    private func endpoint(type: MediaType, category: MediaCategory) -> String {
        ...
        case .movie:
            ...
            case .onTheAir: return "/movie/popular"
            case .airingToday: return "/movie/popular"
            }
        case .tv:
            switch category {
            ...
            case .nowPlaying: return "/tv/popular"
            case .upcoming: return "/tv/popular"
            }
        }
    }
    ...
    func fetchMediaPage(type: MediaType, category: MediaCategory, page: Int) async throws -> MediaPageResult {
        ...
        return try decodeMediaPage(data: data, type: type)
    }

    // Pesquisa real pelo TMDB com paginação
    func searchMedia(query: String, type: MediaType, page: Int) async throws -> MediaPageResult {
        let lang = "pt-PT"
        let ep = type == .movie ? "/search/movie" : "/search/tv"
        let encoded = query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? query
        let urlString = "\(baseURL)\(ep)?api_key=\(apiKey)&language=\(lang)&query=\(encoded)&page=\(page)"
        guard let url = URL(string: urlString) else { throw URLError(.badURL) }
        let (data, _) = try await URLSession.shared.data(from: url)
        return try decodeMediaPage(data: data, type: type)
    }

    private func decodeMediaPage(data: Data, type: MediaType) throws -> MediaPageResult {
        if type == .movie {
